...
if echo hello | zstd -v -T2 2>&1 > $INTOVOID | $GREP -q 'multi-threading is disabled'; then
	hasMT=""
else
	hasMT="true"
fi
...
rm -f tmpOut
zstd tmp -c -o tmpOut > tmpCompressed
...
zstd tmpPrompt - -o tmpPromp.zst --rm < tmpPrompt && die "should have aborted immediately and failed to remove"
...
rm tmp2.zst
cp tmp tmp2
zstd --rm tmp2 -c -o tmp2.zst > $INTOVOID
test ! -f tmp2
...
zstd -c hello.tmp --no-check > hello.zst
zstd -c world.tmp --no-check > world.zst
